...
//delete_option('gc_import_status');
//delete_option('gc_import_batch_number');
//delete_option('gc_import_nbr_comment_import');
//delete_option('gc_import_error_msg');
...
class GcImportService
{
  ...
  public static $options_prefix = 'gc_import_';
  ...
  private function setImportStatus($status) {
    if (in_array($status, GcImportService::$allowedStatus)) {
      update_option(GcImportService::$options_prefix . 'status', $status);
      return true;
    }
    return false;
  }
  ...
  public function setImportErrorMsg($msg) {
    update_option(GcImportService::$options_prefix . 'error_msg', $msg);
  }

  private function deleteImportErrorMsg() {
    delete_option(GcImportService::$options_prefix . 'error_msg');
  }

  private function setImportBatchNumber($batch_number) {
    update_option(GcImportService::$options_prefix . 'batch_number', $batch_number);
  }

  private function getImportStatus() {
    return get_option(GcImportService::$options_prefix . 'status', '');
  }

  private function getImportBatchNumber() {
    return get_option(GcImportService::$options_prefix . 'batch_number', '');
  }

  private function setTotalComments($nbr) {
    update_option(GcImportService::$options_prefix . 'total', $nbr);
  }

  public function getAjaxAdvancement() {
    $nbr_comment_import = get_option(GcImportService::$options_prefix . 'nbr_comment_import', 0);
    $percent = intval($nbr_comment_import) / intval(get_option(GcImportService::$options_prefix . 'total', 100)) * 100;
    return array('nbr_comment_import' => $nbr_comment_import, 'percent' => $percent, 'status' => get_option(GcImportService::$options_prefix . 'status'));
  }

  private function getNbrCommentsImport() {
    return get_option(GcImportService::$options_prefix . 'nbr_comment_import', 0);
  }

  private function setNbrCommentsImport($nbr) {
    update_option(GcImportService::$options_prefix . 'nbr_comment_import', $nbr);
  }

  private function addNbrCommentsImport($nbr) {
    $tmp = get_option(GcImportService::$options_prefix . 'nbr_comment_import');
    $tmp = intval($tmp) + intval($nbr);
    $this->setNbrCommentsImport($tmp);
  }

  private function setDateBegin() {
    update_option(GcImportService::$options_prefix . 'date_begin', date('y-m-d H:i:s'));
  }

  private function setDateEnd() {
    update_option(GcImportService::$options_prefix . 'date_end', date('y-m-d H:i:s'));
  }
  ...
  public function importInit() {
    $this->status = get_option(GcImportService::$options_prefix . 'status');
    ...
    delete_option(GcImportService::$options_prefix . 'status_stopped');
  ...
  public function stopImportation() {
    ...
    if ($this->getImportStatus() !== 'pending') {
      return __('Import Not Pending', 'graphcomment-comment-system');
    }

    if (!empty($batch_number)) {

      $res = GcApiService::importCancel($public_key, $batch_number);

      if ($res['error'] !== false) {
        return $res['error']; // importCancel returns directly the translated error
      }

      update_option(GcImportService::$options_prefix . 'status_stopped', 'true');
      delete_option(GcImportService::$options_prefix . 'status');
      delete_option(GcImportService::$options_prefix . 'import_batch_number');
      delete_option(GcImportService::$options_prefix . 'import_nbr_comment_import');
      delete_option(GcImportService::$options_prefix . 'import_error_msg');
      $this->deleteImportErrorMsg();
      delete_option(GcImportService::$options_prefix . 'import_total');
  ...
  private function finishImport() {
    $public_key = get_option('gc_public_key');
    $batch_number = get_option(GcImportService::$options_prefix . 'batch_number');
